...
export default function TestGider() {
    ...
    const downloadTemplate = async () => {
        try {
            ...
            XLSX.writeFile(wb, "gider_sablon.xlsx");
        } catch (err) {
            console.error(err);
    ...
    const startScan = async () => {
        ...
        try {
            ...
            if (!sheetName) {
                setError(
                    `Bu Excel'de "ŞABLON" adında bir sayfa bulunamadı. Bulunan sayfalar: ${wb.SheetNames.join(", ")}`
                );
                return;
            }
            ...
            if (cariIdx === -1) {
                setError(`"Cari Unvan" başlığı bulunamadı.`);
                return;
            }
            if (hesapIdx === -1) {
                setError(`"Hesap Adı" başlığı bulunamadı.`);
                return;
            }
            ...
            rows = rows.map((r) => {
                ...
                if (rec) {
                    ...
                    unknown += 1;
                    // Eşleşme yoksa orijinal metni bırak; ID alanını boşaltma!
                    copy[hmIdx] = "";
                    copy[hesapIdx] = r[hesapIdx];  // ORİJİNAL HESAP ADINI KORU
                }
                ...
                if (firm) {
                    ...
                    // Eşleşme yoksa ID dönüştürme yapma, orijinal metni koru
                    copy[cariIdx] = cariCell;
                }
    ...
    const sendToReel = async () => {
        try {
            ...
            if (missing.length) {
                setError(
                    `Eksik kolon(lar): ${missing.join(
                        ", "
                    )}. Lütfen "Taramayı Başlat" adımından sonra deneyin.`
                );
                return;
            }
            ...
            // 🔴 TEST ortamı için proxy endpoint
            // TEST yolu artık /api/reel-api/tmsdespatchincomeexpenses/testaddexpense
            const endpoint = "/api/reel-api/tmsdespatchincomeexpenses/test/testaddexpense";
            let ok = 0, fail = 0;
            ...
            for (let r = 0; r < previewRows.length; r++) {
                ...
                const currentAccountId = toPosInt(row[iCari]);
                const lineMovementType = toPosInt(row[iHM]);
                const lineMovementId = toPosInt(row[iHesapAdi]);

                if (!tmsDespatchId || !currentAccountId || !lineMovementType || !lineMovementId) {
                    console.warn(
                        `[TEST] Satır ${r + 1} atlandı: zorunlu ID alan(lar) sayı/pozitif değil`,
                ...
                if (unitPriceNum == null || unitPriceNum <= 0) {
                    console.warn(
                        `[TEST] Satır ${r + 1} atlandı: Geçerli Birim Fiyat zorunlu (> 0). RAW=`,
                        row[iBirimF]
                    );
                    fail++;
                    continue;
                ...
                try {
                    ...
                    if (!res.ok) {
                        ...
                        console.error(`[TEST] Satır ${r + 1} hata:`, res.status, txt);
                        fail++;
                    } else {
                        ok++;
                    }
                } catch (err) {
                    console.error(`[TEST] Satır ${r + 1} istek atılamadı:`, err);
                    fail++;
                }
            }

            alert(`TEST gönderim tamamlandı ✅ Başarılı: ${ok} · Hatalı: ${fail}`);
        } catch (e) {
            console.error(e);
                ...
                {previewHeaders.length > 0 && (
                    <div className="ge-table-wrap">
                        <div className="ge-table-top">
                            <span>
                                Önizleme: <strong>{previewRows.length}</strong> satır
                            </span>
                                    ...
                                    <thead>
                                        <tr>
                                            {previewHeaders.map((h, i) => (
                                                <th key={i} className={missingHeaders.includes(h) ? "miss" : ""}>
                                                    {h}
                                                </th>
                                            ))}
                                        </tr>
