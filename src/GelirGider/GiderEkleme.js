...
import { FiUploadCloud, FiFile, FiCheckCircle, FiXCircle, FiAlertTriangle, FiTrash2, FiDownload, FiSearch, FiSend, FiRefreshCw, FiTrendingDown } from 'react-icons/fi';
...
export default function GiderEkleme() {
    // --- State'ler ---
    ...
    const [rowResults, setRowResults] = useState({}); // Yeni: Satır bazlı gönderim sonuçları
    const [sendSummary, setSendSummary] = useState(null); // Yeni: Toplu gönderim özeti
    ...
    const validateAndSet = (f) => {
        ...
        if (!ok) {
            const msg = "Lütfen .xlsx veya .xls uzantılı bir dosya yükleyin.";
            setError(`❌ ${msg}`);
        ...
        setMissingHeaders([]);
        setMapStats({ matched: 0, unknown: 0 });
        clearSendState(); // Yeni: Dosya değişince önceki sonuçları temizle
    };
    ...
    // ✅ Wizard'dan gelen Excel'i otomatik alma (postMessage)
    useEffect(() => {
        ...
        const onMessage = (event) => {
            ...
            if (!msg || msg.type !== "AUTO_UPLOAD_EXCEL") return;
            if (hasAutoUploadedRef.current) return;
            hasAutoUploadedRef.current = true;


            try {
                const { fileName, mime, buffer } = msg;

                // ArrayBuffer -> File
                const blob = new Blob([buffer], { type: mime });
                const incomingFile = new File([blob], fileName || "otomatik.xlsx", { type: mime });

                // Senin mevcut akışın: dosyayı UI'a set et
                validateAndSet(incomingFile);

                // URL'de ?autoscan=1 varsa taramayı otomatik başlat
                const url = new URL(window.location.href);
                const autoScan = url.searchParams.get("autoscan") === "1";
                if (autoScan) {
                    setTimeout(() => startScan(incomingFile), 0);
                }

                setNotification({
                    message: `Dosya otomatik alındı: ${incomingFile.name}${autoScan ? " (tarama başlatıldı)" : ""}`,
                    type: "success",
                });
            } catch (e) {
                console.error(e);
                setNotification({ message: "Otomatik dosya alma hatası.", type: "fail" });
            }
        };
    ...
    const sendToReel = async () => {
        try {
            ...
            if (missing.length) {
                const msg = `Eksik kolon(lar): ${missing.join(", ")}. Lütfen "Taramayı Başlat"tan sonra deneyin.`;
                setError(`❌ ${msg}`);
                setNotification({ message: msg, type: 'fail' });
                return;
            }
            ...
            const toNumber = (val) => {
                const s = toDecimalString(val, 6); // toFixed(2) yerine 6 hane kullanın
                if (s === "") return null;
                const n = Number(s);
                    ...
                    if (!currentAccountId) {
                        ...
                        details.push(
                            isFirmId
                                ...
                                : `Cari Unvan: "${rawCari ?? ""}" → Firma adı eşleşmedi veya ID'ye çevrilemedi.`
                        );
                    }
    ...
    // --- Render ---
    return (
        <div className="ge-inline-page gider" style={{ ...STYLES.page, position: "relative" }}>
            ...
            <div className="ge-inline-header" style={STYLES.header}>
                <h2 style={STYLES.h2}><FiTrendingDown size={22} color={COLORS.primary} />Gider Kayıt Aktarımı</h2>
                <button
                ...
                {!file ? (
                    <div
                        style={{
                            ...
                            backgroundColor: isDragging ? 'rgba(220, 53, 69, 0.05)' : 'transparent' // Gider rengi
                        }}
                    ...
                    <div className="ge-inline-file" style={{
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            ...
                            <button
                                style={{
                                    ...STYLES.buttonBase, ...STYLES.buttonSuccess, // Başarılı butonu yine yeşil kalsın
